...
const translations = {
  'zh-CN': {
    ...
    hero_sub: '本地优先、隐私优先、轻量低占用的编程 Agent 桌面伴侣：把 Hook 活动转化为像素风桌宠的细微回应，不打扰你的工作流。',
    ...
    feat_agent_desc: '连接 Claude Code、Codex CLI、Qoder 与 Antigravity，桌宠实时反映 Agent 活动。',
    ...
    status_available: '可用',
    perf_title: '性能表现',
    perf_desc: '对应 UI 隐藏时暂停轮询，只发送实际变化的渲染状态。测试设备：MacBook Pro（Apple M3 芯片）。测试方法：连续运行约 2 小时 39 分钟后，进行 30 秒空闲采样：',
    stat_cpu: '平均 CPU 占用',
    stat_mem: '物理内存占用',
    privacy_title: '隐私优先',
  ...
  'en-US': {
    ...
    hero_sub: 'A local-first, privacy-first, lightweight desktop companion for coding agents: it turns hook activity into subtle pixel-art reactions without taking over your workspace.',
    ...
    feat_agent_desc: 'Connect Claude Code, Codex CLI, Qoder, and Antigravity so the pet mirrors their activity.',
    ...
    perf_desc: 'Familiar pauses polling while the UI is hidden and only sends render state when it changes. Tested on a MacBook Pro (Apple M3 chip): a 30-second idle sample after ~2h39m of continuous runtime.',
